...
Host/capture/screensaver recipe is `.cursor/skills/sunshine-ds-gamemode/SKILL.md` (`checkpoint-2026-09-10-gamemode-dual-stream`). Do **not** run the KWin desktop script in Game Mode. Use `scripts/ensure-azahar-gamemode-dual-screen.sh` (standalone Azahar, SteamLaunch, Secondary Window `ffplay` `x11grab` onto `:2`). Default pad is **Odin** (`AZAHAR_PAD_MATCH`). Restart Azahar after a GUID change. After `ensure-sunshine-ds-gamemode.sh --start`, re-run `--mirror-only` (kms recreates `:2` and the idle screensaver covers the 3DS bottom). On Azahar/mirror exit, `sunshine-ds-gamemode-virtual.sh --paint` restores the clock on video/1. Overlay is the Cemu recipe when Steam Big Picture is mapped; otherwise sunshine-ds sends `steam://overlay/toggle`. Do **not** start `steam-guide-from-select.py`.
