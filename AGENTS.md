...
- RPCS3 player 1 → current pad (`ensure-rpcs3-input.sh`: user-slot wrapper + `rpcs3` on RetroDECK PATH; Device is SDL pad name, emulated Sixaxis VID/PID stays). RetroDECK RPCS3 is **SDL3**: it hides Steam virtual `28de:11ff` unless `SDL_GAMECONTROLLER_ALLOW_STEAM_VIRTUAL_GAMEPAD=1` is in the environment (SetHint is not enough). Null players 2–7. Do not bind Sunshine’s ghost Xbox as player 1 during local play.
