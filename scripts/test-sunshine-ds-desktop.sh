...
VIRT_W="${SUNSHINE_DS_VIRTUAL_WIDTH:-1920}"
VIRT_H="${SUNSHINE_DS_VIRTUAL_HEIGHT:-1080}"
...
kscreen_json() {
  # Duplicate Virtual-sunshine-ds outputs make kscreen-doctor hang. Bound it.
  timeout 8 kscreen-doctor -j 2>/dev/null | python3 -c 'import json,re,sys; s=sys.stdin.read(); s=re.sub(r"\x1b\[[0-9;]*m","",s); json.dump(json.loads(s), sys.stdout)'
}
...
ensure_virtual_output() {
  ...
  timeout 8 kscreen-doctor -o || true
  echo "helper log:"
...
def doctor():
    raw = subprocess.check_output(["timeout", "8", "kscreen-doctor", "-j"], text=True)
...
def geom(data, name):
    for o in data.get("outputs", []):
        ...
        w = int(round((size.get("width") or 1920) / scale))
        h = int(round((size.get("height") or 1080) / scale))
