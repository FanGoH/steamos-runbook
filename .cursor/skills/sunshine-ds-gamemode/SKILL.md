...
- **4K HDMI + 1080p GamePad 1/4 flicker** (Thor clip `screen-20260917-140731`, also in the Steam menu): steamcompmgr on nested `:1` (xwayland idx ≠ 0) is SteamControlled. Every opaque InputOutput window is a HDMI focus candidate (`appID` falls back to the xid). GamePad damage steals the 4K TV layer → 1080p top-left quarter. Shrinking `:1` to 1080p (`MODE_CONTROL` `1,1920,1080,0`) integer-scales Steam itself and still flickers in the menu. Opacity 0 on the GL child alone drops it from focus and kills taps unless it stays top of the **X11** stack. Parking at `3840,0` clamps `XWarpPointer` (wx ignores `send_event`). Fix: keep `:1` at HDMI native (`MODE_CONTROL` idx 1 = 3840×2160). TV fills that. GamePad stays 1920×1080 at `0,0`. Tag the frame **and** GL children `GAMESCOPE_EXTERNAL_OVERLAY=1` plus `_NET_WM_WINDOW_OPACITY=0` so they are not the HDMI layer and are not painted as an overlay. `windowraise` (not `windowactivate`) so QueryPointer/warp still hit the child. Stamp `GAMESCOPECTRL_BASELAYER_WINDOW` on **`:0`** to the TV. Restore MODE_CONTROL to HDMI size when the emulator exits. Do not set idx 2.
...
- Cemu TV ≥64×64 InputOutput on session **`:1` at HDMI native** (`GAMESCOPE_XWAYLAND_MODE_CONTROL` `1, 3840, 2160, 0` on a 4K TV). GamePad View is the sibling at `0,0` 1920×1080 with `GAMESCOPE_EXTERNAL_OVERLAY=1` and opacity 0 on the frame **and** GL child, then `windowraise` (inject target). Steam BPM stays 4K on `:0`. A 10×10 InputOnly `Cemu_relwithdebinfo` is the hidden helper — do not make it HDMI BASELAYER. GamePad inject log must be `using :1`. HDMI / top taps are a **separate** display-0 inject onto the TV GL child (~3840×~2160 minus chrome). Do not route top taps through GamePad View. Stamp BASELAYER on `:0`.
...
- Opacity 0 on the Cemu/Azahar **GL child** without `GAMESCOPE_EXTERNAL_OVERLAY` **and** `windowraise` — focus skip hides HDMI but QueryPointer hits the TV and taps die
