...
- Switch on this box: install Eden into RetroDECK’s **user** slot (`/var/data/retrodeck/external_components/eden/`) via `scripts/ensure-eden-component.sh`. Do **not** use [iAbuser/RetroInjector](https://github.com/iAbuser/RetroInjector) (sudo-copies cooker tarballs into the Flatpak `files/` dir; wiped on Flatpak update). ES-DE must use `%EMULATOR_EDEN%` — **not** `%EMULATOR_FLATPAK-SPAWN% --host`. Host spawn drops Game Mode env and makes overlay/Exit attach to RetroDECK while Eden is a foreign process. `%INJECT%` (old Ponzu/Yuzu) only adds per-game argv from a `.esprefix` file. Ponzu is gone as of RetroDECK 0.10. In-sandbox Eden must use Steam’s virtual pad (`SDL_GAMECONTROLLER_ALLOW_STEAM_VIRTUAL_GAMEPAD=1`, hidapi off, ignore ASRock LED `26ce:01a2`, player 0 GUID `030079f6de280000ff11000001000000` like Cemu `SteamInput-P1` / Azahar). Disable Eden `enable_joycon_driver` so it does not steal the Xbox HID (`hid_read failure` kills the virtual pad). Do not bind player 0 to the physical Xbox GUID.
