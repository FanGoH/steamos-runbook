...
| [FanGoH/Sunshine](https://github.com/FanGoH/Sunshine) | `cursor/bottom-touch-4k-f15e` **`4ca50111`** (XTest GamePad taps + linear Stretch `packet_to_unit`). HDMI Eden top taps are a **trackpad** (`a9451958` on `cursor/eden-hdmi-touch-f15e`, no LEFT hold); Cemu/Azahar HDMI and Steam overlay stay abs. AUTOCONNECT + object.serial remains `9e07d39e` — do not ship `f8d9968c` skip-AUTOCONNECT |
...
| Host ELF | `~/.local/bin/sunshine-ds-kms` git **`a9451958`** (Eden HDMI trackpad, no LEFT hold, on `4ca50111` GamePad), `cap_sys_admin=ep`, RUNPATH `~/.local/lib/sunshine-ds-kms`. Sidecar must not write `node=` |
| Sidecar | `$XDG_RUNTIME_DIR/sunshine-ds-gamemode-virtual`: `serial=` + **`pw_node=`** (not `node=`). `f8d9968c` skips AUTOCONNECT when `node=` is set |
| Conf | `capture = kms`, `output_name = HDMI-A-1`, `dual_display_source = gamescope-virtual`, `port = 48200`, `encoder = vaapi`, `minimum_fps_target = 60`, `gamepad = x360`, `back_button_timeout = 500`, `audio_sink` = HDMI alsa leaf (Steam UI + VSS mix; not VSS.monitor, not `sink-sunshine-stereo`). Host ELF assets path patched `/usr/local/assets` → `/home/deck/sdsast` |
| Moonlight | host **`:48200`** uniqueid `1075C8EF…`. Desktop app **`958645192`**. Not `881448767`, not Decky `:47989`, not `:48100` |
| Idle bottom | `scripts/sunshine-ds-bottom-screensaver.py` on `:2` (clock + moving bar). `--paint` hops via `systemd-run --user --no-block` after dropping leftover `sunshine-ds-bottom-paint` fragment (do not start the clock in `app-steam-app*.scope`) |
| Cemu | Tender Play while `:48200` BUSY + virtual sidecar → `CEMU_GAMEMODE_DS=1` + `--attach` (**no** `-f`). Manual: `CEMU_PAD_MATCH=Thor ./scripts/ensure-cemu-gamemode-dual-screen.sh`. TV + GamePad View on session `:1` (`FOCUS_DISPLAY=1`). `ffplay -window_id` **must** `-i :1.0` (not `:0.0`) onto `:2` with `-vf scale=1920:1080`. A kms restart can remap that ffplay at **640×480** — recover/`present_virtual_gamepad` must `windowsize` 1920×1080. HDMI TV is native (4K). Do not `$(find_pad_wid)` — that drops `TV_DISPLAY`. Exit: `--quit` on first tick, then `--paint` |
...
| GamePad tap | Inject opens `:1` then `:0` (never `$DISPLAY` / `:2`). Cemu `FOCUS_DISPLAY=1` puts GamePad View on `:1` at 1920×1080 `0,0` while `:1` is HDMI native (4K). wx/GTK drop `XSendEvent`; overlay-tag + opacity 0 skip gamescope hit-test — briefly un-hide, `XTestFake*`, re-cover. Flush FOCUS_DISPLAY writes on `:0`. Do not also uinput. Thor Stretch fills 1240×1080; live packets are `ref=1239x1079` (Moonlight sends view size, protocol stores width-1). `packet_to_unit` is linear `x/width` (Stretch). Do **not** unletterbox — that mapped packet y=190 to unit.y≈0. Fit already shrinks StreamView to 16:9. New client stream-sized ref is the same linear map. Log `GamePad pkt` then `abs unit=` (live: `x=165,332 ref=1239x1079 unit=0.133,0.308`). Host `4ca50111`. Tag **`checkpoint-2026-09-18-gamepad-xtest`** (user: “WE DID IT”) |
| HDMI / top tap | Tag **`checkpoint-2026-09-10-gamemode-top-touch`**. Display 0: Cemu TV / Azahar on `:1` stay window-local abs; Steam Big Picture on `:0` while hold-Select overlay is up (`STEAM_OVERLAY=1`, log `HDMI inject: overlay on :0` / `overlay-abs`). **Eden** (`Eden | v…`, often 1024×576 on 4K `:1`) is a **trackpad**: `inject_hdmi_surface_abs_mouse` returns false, then `HDMI trackpad dx=` with **no LEFT hold**. Moonlight holds LEFT for the whole finger; swallow it (`finger-down (no hold)`) and click only on a short lift (`finger-up … tap`). Do not XTest the full HDMI stream onto that Qt window. Overlay hide still matches Eden as HDMI BASELAYER. Do not take this path for Odin GamePad-only. GamePad path stays display 1 (absolute XTest clicks). Live 17:34 in-game Cemu: `HDMI inject: using :1` TV `0x80003f` 1920×1051; bottom `GamePad inject: using :1` `0x800040`. Live 17:43 overlay (user: “touches on top work”): `HDMI inject: overlay on :0` then `overlay-abs … xid=25165829 1920x1080` |
| Thor pad | Bind **EmuPads P1** (`1209:e301`). Mux copies Thor/Odin/phone. Wait for the Sunshine pad with `wait-appear` if it is not up yet; do not write Thor’s GUID into XML. Cemu uuid `{sdl-index}_{guid}` — restart Cemu after bind if index changed. `_X360_SDL_MAP` is **15-button** libvirtualhid (`LB=b6` `Back=b10` `Start=b11`); do **not** write 11-button Steam xpad (`LB=b4` `Back=b6`). WW first-person look is **L bumper** (Wii U L), not Select |
...
| [FanGoH/Sunshine](https://github.com/FanGoH/Sunshine) | `cursor/eden-hdmi-touch-f15e` | `a9451958` | Eden HDMI trackpad without LEFT hold (tap-click on short lift); Cemu/Azahar HDMI + overlay stay abs; GamePad path unchanged |
