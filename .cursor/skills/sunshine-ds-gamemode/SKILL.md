...
| Audio | Pulse default HDMI (Steam UI; `STEAM_DISABLE_AUDIO_DEVICE_SWITCHING=1`). Cemu Cubeb → **Virtual Surround Sound** → that HDMI | Moonlight captures the **HDMI** sink monitor (leaf), not VSS and not `sink-sunshine-stereo`. VSS.monitor is Cemu-only — Steam menu clicks never appear there. Log: `Capturing host sink [alsa_output…hdmi…]` / `Found default monitor by name: …hdmi….monitor`. |
