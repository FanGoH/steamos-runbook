...
## Game Mode (`:48200`) — checkpoint `checkpoint-2026-09-14-gamemode-fill-exit`

Host/HUD (4K HDMI + QAM preset 2 + video/1 clock rebind) is **`checkpoint-2026-09-17-hdmi-hud`**. GamePad taps are **`checkpoint-2026-09-18-gamepad-xtest`** (user: “WE DID IT”). Prefer **`checkpoint-2026-09-14-gamemode-fill-exit`** for Tender Cemu tiles: they fill Thor bottom (`ffplay` `scale=1920:1080` onto `:2`; Moonlight Stretch/Fit on 1080p). HDMI stays native (4K). Tile grab is still `checkpoint-2026-09-11-gamemode-tender-ds` (user: “amazing, cemu works correctly” — `ffplay` x11grab from session `:1`, not `:0`). Overlay/QAM/Home mute EmuPads sinks; Steam Exit `--quit`s on SIGTERM only (`checkpoint-2026-09-11-steam-menu-mute`). Earlier `checkpoint-2026-09-10-gamemode-tender-ds` attached but grabbed `:0` for a `:1` xid. Do not “improve” this unless it breaks. Full recipe: `.cursor/skills/sunshine-ds-gamemode/SKILL.md`. Screens-only subset: `checkpoint-2026-09-10-gamemode-dual-stream`. Sep 9 `checkpoint-2026-09-09-gamemode-cemu-ds` is the HDMI DCC / Cemu-picture baseline.
...
`--stop` kills the pad mirror and the gamescope focus watcher only. Overlay/QAM/Home/Library mute EmuPads sinks. Steam Exit `--quit`s **only** on SIGTERM pending. Do **not** `--quit` on `FOCUSED_APP=769` (that killed Cemu when opening Home; `log.txt` stopping mid-`FSGetVolumeState` is SIGTERM, not a Cemu crash). Moonlight Quit / `sunshine-app-stop.sh cemu` / `--quit` end the game.
