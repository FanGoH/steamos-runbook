#!/usr/bin/env bash
# Install / refresh the FGPC Decky plugin (QAM catalog for fgpc).
...
PLUGIN_DEST="${DECKY_HOMEBREW_DIR:-/home/$STEAMOS_USER/homebrew}/plugins/FGPC"
SNAPSHOT="${DECKY_HOMEBREW_DIR:-/home/$STEAMOS_USER/homebrew}/data/FGPC"
PARENT="$(dirname "$PLUGIN_DEST")"
PLUGIN_NAME="$(python3 -c "import json; print(json.load(open('$SRC/plugin.json'))['name'])")"

snapshot_backend() {
  mkdir -p "$SNAPSHOT/scripts" "$SNAPSHOT/fgpc/src/fgpc" "$SNAPSHOT/logs"
  cp -a "$ROOT/scripts/fgpc-api.py" "$SNAPSHOT/scripts/"
  cp -a "$ROOT/fgpc/src/fgpc/__init__.py" "$SNAPSHOT/fgpc/src/fgpc/"
  cp -a "$ROOT/fgpc/src/fgpc/api.py" "$SNAPSHOT/fgpc/src/fgpc/"
  cp -a "$ROOT/fgpc/src/fgpc/catalog.py" "$SNAPSHOT/fgpc/src/fgpc/"
  cp -a "$ROOT/fgpc/src/fgpc/invoke.py" "$SNAPSHOT/fgpc/src/fgpc/"
  cp -a "$ROOT/fgpc/src/fgpc/playbook.py" "$SNAPSHOT/fgpc/src/fgpc/"
}
...
copy_plugin() {
  ...
  cp -a "$SRC/main.py" "$SRC/plugin.json" "$SRC/package.json" "$SRC/README.md" "$PLUGIN_DEST/"
  cp -a "$SRC/dist/index.js" "$PLUGIN_DEST/dist/index.js"
}

if [ ! -d "$PARENT" ]; then
  echo "Decky plugins dir not found ($PARENT)."
  record_manual "Install Decky Loader, then FGPC" <<EOF
# After Decky exists:
$ROOT/scripts/ensure-fgpc-decky.sh
EOF
  exit 2
fi
...
if [ -w "$PARENT" ]; then
  copy_plugin
  finish_install "Installed FGPC to $PLUGIN_DEST"
fi
...
if [ -w "$PLUGIN_DEST/main.py" ] && [ -w "$PLUGIN_DEST/dist/index.js" ]; then
  ...
  [ -w "$PLUGIN_DEST/README.md" ] && cp -a "$SRC/README.md" "$PLUGIN_DEST/README.md"
  finish_install "Updated writable FGPC files in $PLUGIN_DEST"
fi

if sudo -n true 2>/dev/null; then
  sudo mkdir -p "$PLUGIN_DEST/dist"
  sudo cp -a "$SRC/main.py" "$SRC/plugin.json" "$SRC/package.json" "$SRC/README.md" "$PLUGIN_DEST/"
  sudo cp -a "$SRC/dist/index.js" "$PLUGIN_DEST/dist/index.js"
  finish_install "Installed FGPC to $PLUGIN_DEST (sudo)"
fi
