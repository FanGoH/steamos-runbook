...
**Checkpoint (user 2026-09-11: “amazing, cemu works correctly”): `checkpoint-2026-09-11-gamemode-tender-ds`.** Tender Cemu **tiles** while `:48200` is BUSY launch windowed dual-screen (`--attach`, no `-f`): TV on HDMI, GamePad `ffplay` on Thor bottom. Cemu lives on session `:1` (`FOCUS_DISPLAY=1`); finders must set `TV_DISPLAY` in this shell — `$(find_pad_wid)` drops it and x11grab uses `:0` (`Can't find window`; bottom stays the clock). Overlay/QAM SIGSTOP; Steam Exit `--quit`s on the first tick (SIGTERM pending or `FOCUSED_APP=769` after 8s) so Exiting… does not wait. Leftover `:2` x11grab dies before `--paint`. Earlier: `checkpoint-2026-09-10-gamemode-tender-ds` (Tender Play attach, but tile GamePad grab was still `:0`). Local Play (kms FREE) stays `-f`. Steam menu clicks still **pending**. Do not “improve” it unless it breaks.
...
| Overlay | Hold Select 0.5s → `STEAM_OVERLAY=1` on BPM + `FOCUSED_APP=769` | Same; hide restores Cemu TV. **QAM** is Quick Access (`...`): `GAMESCOPE_BLUR_MODE!=0`, FOCUSED_APP stays the game — do not treat 769 as QAM, do not `keepAbove`/raise the emu over that panel. Overlay lists Cemu TV + GamePad View. Focus watcher must not reclaim while overlay or QAM is up. Overlay/QAM **SIGSTOP** Cemu / Azahar / Eden (`scripts/inhibit-emu-input-on-steam-ui.py`). Steam Exit `--quit`s on the first tick (SIGTERM pending, or `FOCUSED_APP=769` after 8s) — do not SIGSTOP then wait. After the emulator dies, restore `FOCUSED_APP=769` on `:0` (leave `:1` if Cemu is still there). Steam keeps the Sunshine pad for overlay / QAM. |
