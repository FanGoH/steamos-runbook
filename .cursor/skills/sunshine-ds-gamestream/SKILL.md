...
| Game Mode `:48200` **Cemu audible, Steam menu silent** | Steam has `STEAM_DISABLE_AUDIO_DEVICE_SWITCHING=1` and plays UI to the Pulse default HDMI sink. A pinned `audio_sink = Virtual Surround Sound` records Cemu only. `write_kms_conf` pins the HDMI alsa sink instead. |
