...
- Game Mode `:48200` dual-stream **THE standard** (2026-09-18, tag **`checkpoint-2026-09-18-gamepad-xtest`**, user: “CHECKPOINT, WE DID IT”): Thor bottom taps reach Cemu GamePad and line up under Stretch (XTest + `:0` FOCUS_DISPLAY flush; Stretch `ref=1239x1079` maps linear `x/width`, do not unletterbox). HUD subset `checkpoint-2026-09-17-hdmi-hud`: 4K HDMI + QAM **preset 2** bar spans the TV (mangoapp overlay **3840×2160**; headless `:2` isolated mangoapp ftok). After `:2` respawns, kms rebinds video/1 so Thor bottom is the clock, not HDMI. Cemu/Azahar fill+Exit remains `checkpoint-2026-09-14-gamemode-fill-exit` (Thor **dual-panel** fills the bottom: `ffplay` `scale=1920:1080`; Moonlight GamePad Stretch/Fit on 1080p, `faf41b72`. Azahar Exit hops `--paint` outside Steam). Tile grab remains `checkpoint-2026-09-11-gamemode-tender-ds` (user: “amazing, cemu works correctly” — windowed `--attach`, no `-f`, GamePad `ffplay` grabs session `:1`). Overlay/QAM/Home mute EmuPads sinks; Steam Exit `--quit`s on SIGTERM only (`checkpoint-2026-09-11-steam-menu-mute`). Host `sunshine-ds-kms` (`cap_sys_admin=ep`), `capture = kms`, `dual_display_source = gamescope-virtual`. Sidecar `serial=` + `pw_node=` (not `node=`). Recipe: `.cursor/skills/sunshine-ds-gamemode/SKILL.md`. HDMI DCC: `[kmsgrab] DMA-BUF copied`. video/1: `Connect PW stream PW_ID_ANY serial=` + `cpu frame type=2`. Earlier: `checkpoint-2026-09-10-gamemode-tender-ds` (tile grab still `:0`). `checkpoint-2026-09-10-gamemode-cemu-audio` (manual script). Screens-only: `checkpoint-2026-09-10-gamemode-dual-stream`. GamePad / bottom touch: `checkpoint-2026-09-18-gamepad-xtest` (`GamePad pkt` linear Stretch; XTest). HDMI / top touch: tag `checkpoint-2026-09-10-gamemode-top-touch` (user: “touches on top work”). In-game `HDMI inject: using :1` onto TV GL child; hold-Select overlay `HDMI inject: overlay on :0` onto Steam BPM. GamePad path unchanged. Sep 9 `checkpoint-2026-09-09-gamemode-cemu-ds` (`119d7452`) is the DCC/Cemu-picture baseline. Do not merge kms into play / `:48100`. Azahar Game Mode (`970550cd`) injects onto **Secondary Window** and restores **Primary Window**.
...
| Game Mode `:48200` **bottom pitch black**, video/1 `connecting`, software DMA-BUF `nonzero=0`, gamescope node `suspended` / zero links | Live `sunshine-ds-kms` from `f8d9968c` skipped AUTOCONNECT when the sidecar had `node=`. Sep 9 (`119d7452`) is `PW_KEY_TARGET_OBJECT` = object.serial + `PW_STREAM_FLAG_AUTOCONNECT`. Sidecar must write `serial=` / `pw_node=` (not `node=`). Host `gst-launch pipewiresrc target-object=$serial` (`--smoke`) must produce a PNG before blaming kms. If a *new* headless gamescope never logs `stream available on node ID`, restart PipeWire on the **user** bus (not `sudo systemctl --user`), then `--paint`. Do not skip AUTOCONNECT just because a node id exists. |
...
Game Mode `:48200` is **THE standard** `checkpoint-2026-09-18-gamepad-xtest`. Recipe: `.cursor/skills/sunshine-ds-gamemode/SKILL.md`. Moonlight already on `sunshine-ds-kms` (not Decky). HUD subset **`checkpoint-2026-09-17-hdmi-hud`**; Cemu fill+Exit subset **`checkpoint-2026-09-14-gamemode-fill-exit`**. Desktop app is `958645192`. Last client gone must leave Desktop `FREE`; then tap **Desktop** or the Tender Cemu tile (Wind Waker HD). While `:48200` is BUSY and `$XDG_RUNTIME_DIR/sunshine-ds-gamemode-virtual` has `serial=` / `pw_node=`, Tender Play sets `CEMU_GAMEMODE_DS=1` and `--attach` (no `-f`). GamePad is on session `:1`; x11grab must use that display and `scale=1920:1080`. Local Play (kms FREE) stays `-f`. Manual: `CEMU_PAD_MATCH=Thor ./scripts/ensure-cemu-gamemode-dual-screen.sh`. Close a leftover Tender `-f` Cemu with `scripts/sunshine-app-stop.sh cemu` before the first streaming Play.
...
- Write sidecar `node=` (use `pw_node=`). `f8d9968c` then skips AUTOCONNECT and Thor bottom stays black
- `ensure-sunshine-ds-gamemode.sh --start` while headless `:2` is already up (`KillMode=mixed` can kill the helper). Restart kms with `--start-kms` or `systemctl --user restart steamos-sunshine-ds-gamemode.service`
