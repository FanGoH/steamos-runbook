...
- **4K HDMI + 1080p GamePad 1/4 flicker** (Thor clip `screen-20260917-140731`, **also in the Steam menu**): not only Cemu. Nested `:1` must stay HDMI native — shrinking it to 1080p quarters Steam itself. `mangoapp overlay window` **is Steam’s performance overlay** (Settings → Monitor performance), not Hold-Select / QAM. Do **not** set opacity 0 on it. A leftover 1080p mango window on 4K HDMI can 1/4-flash; size the mapped overlay to HDMI (`gamescope_show_mangoapp`). Cemu GamePad is a second 1080p focus candidate on `:1`; overlay-tag + opacity 0 + `windowraise` (not `windowactivate`). Do not park at `3840,0`. Stamp BASELAYER on **`:0`**. Do not set MODE_CONTROL idx 2.
