...
- Desktop/GameStream Cemu: `scripts/bind-gamepad.py` (`list` / `cemu --match Thor`) then `scripts/ensure-cemu-dual-screen.sh` (live kscreen sizes; KWin-place TV on HDMI + GamePad View on Virtual-sunshine-ds). Standalone Flatpak `info.cemu.Cemu`, type **Wii U GamePad**. Do not hand-edit `controller0.xml`. Optional `.env` `CEMU_ROM` / `CEMU_PAD_MATCH` (default Thor). Skills `.cursor/skills/bind-controller/SKILL.md` and `.cursor/skills/cemu-dual-screen/SKILL.md`.
- Desktop/GameStream Azahar (3DS): `scripts/bind-gamepad.py azahar --match Thor` then `scripts/ensure-azahar-dual-screen.sh`. Standalone Flatpak `org.azahar_emu.Azahar` (not RetroDECK). Separate Windows, `QT_QPA_PLATFORM=xcb`. Primary Window → HDMI, Secondary Window → Virtual-sunshine-ds. x360 L/R = SDL 6/7. Skill `.cursor/skills/azahar-dual-screen/SKILL.md`. Optional `.env` `AZAHAR_ROM` / `AZAHAR_PAD_MATCH`.
