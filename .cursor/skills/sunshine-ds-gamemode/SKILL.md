...
**Checkpoint (user 2026-09-11: “amazing, cemu works correctly”): `checkpoint-2026-09-11-gamemode-tender-ds`.** Tender Cemu **tiles** while `:48200` is BUSY launch windowed dual-screen (`--attach`, no `-f`): TV on HDMI, GamePad `ffplay` on Thor bottom. Cemu lives on session `:1` (`FOCUS_DISPLAY=1`); finders must set `TV_DISPLAY` in this shell — `$(find_pad_wid)` drops it and x11grab uses `:0` (`Can't find window`; bottom stays the clock). Overlay/QAM mute EmuPads sinks; Steam Exit `--quit`s on the first tick (SIGTERM pending or `FOCUSED_APP=769` after 8s) so Exiting… does not wait. Leftover `:2` x11grab dies before `--paint`. Earlier: `checkpoint-2026-09-10-gamemode-tender-ds` (Tender Play attach, but tile GamePad grab was still `:0`). Local Play (kms FREE) stays `-f`. Steam menu clicks still **pending**. Do not “improve” it unless it breaks.
