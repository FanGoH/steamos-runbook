...
- **4K HDMI + 1080p GamePad 1/4 flicker** is GamePad View mapped at `1920×1080+0+0` on the same gamescope as the 4K TV. gamescope composites that opaque region as the top-left quarter even when TV is BASELAYER. Hide it with `_NET_WM_WINDOW_OPACITY=0` (x11grab still reads the pixmap onto `:2`). Do not restack TV/GamePad every tick. A withdrawn Tk clock matching `xdotool search` also made the Cemu watcher relayout; recover drops that clock while the mirror is live.
