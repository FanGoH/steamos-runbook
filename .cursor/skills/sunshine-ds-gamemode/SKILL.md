...
**Checkpoint `checkpoint-2026-09-17-hdmi-hud`:** 4K HDMI + 1080p GamePad/clock. QAM **preset 2** horizontal bar spans the TV (mangoapp overlay **3840×2160**; headless `:2` uses its own `ftok("mangoapp")` cwd). After `:2` respawns, kms `systemctl --user restart`s to rebind video/1 (`serial=` + `pw_node=`) so Thor bottom is the idle clock, not HDMI. Nested `:1` stays HDMI native. GamePad frame-only opacity 0. Tender Cemu/Azahar fill+Exit remains `checkpoint-2026-09-14-gamemode-fill-exit` (`ffplay` `scale=1920:1080` onto `:2`; Moonlight GamePad Stretch/Fit on a 1080p stream, `faf41b72`). Tile grab remains `checkpoint-2026-09-11-gamemode-tender-ds` (`--attach`, no `-f`, x11grab session `:1`). 3DS dumps `checkpoint-2026-09-11-3ds-n3ds`. Home/Library mute `checkpoint-2026-09-11-steam-menu-mute`. Mux `checkpoint-2026-09-11-emupads-mux`. Local Play (kms FREE) stays `-f`. Steam **audio** menu clicks still **pending**.
...
- **4K HDMI + 1080p GamePad bottom taps**: GamePad View stays 1920×1080 at `0,0` on nested `:1` (now 3840×2160). Inject warps **into that window** and XSendEvents the GL child. Do **not** also uinput — gamescope hit-tests the Cemu TV, so a 1080p GamePad click lands in the TV’s top-left quarter (WW item pad / cursor jumps). Live kms must be rebuilt after that inject change (`--replace-bin` / `--promote-new`); a Sep 14 ELF still uinputs. Map the Moonlight packet with `packet_to_unit` (packet x/width) — do **not** run `client_to_touchport` (encode letterbox + HDMI `env_logical`) over Thor’s 1080×1240 panel ref. Moonlight must send **stream** 1920×1080 as the abs-mouse reference (not the panel view size) and route `displayId` to `streamViewSecondary`. Native LI_TOUCH is compiled out; fingers are abs mouse display 1. Log: `GamePad pkt x=… ref=… unit=…` then `abs unit=… px=… 1920x1080`.
