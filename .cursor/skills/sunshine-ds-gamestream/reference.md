...
| GamePad tap | Inject opens `:1` then `:0` (never `$DISPLAY` / `:2`). Cemu `FOCUS_DISPLAY=1` puts GamePad View on `:1` at 1920×1080 `0,0` while `:1` is HDMI native (4K). Warp dest is the GamePad window (not the 4K root). Do not also uinput — that click hits the TV at 1080p coords (top-left quarter). Live ELF must include `packet_to_unit` + no-uinput (`d6b4e23f` and later); Sep 14 kms still uinputs. Moonlight sends stream 1920×1080 as the abs-mouse ref (not Thor’s 1080×1240 panel). Log `GamePad pkt` then `abs unit=`. Tag `checkpoint-2026-09-10-gamemode-gamepad-touch` |
