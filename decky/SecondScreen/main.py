...
class Plugin:
    ...
    async def set_screensaver(self, mode: str = "on", **kwargs: object) -> dict:
        if kwargs:
            mode = str(kwargs.get("mode", mode) or mode)
        script = _script()
        if not os.path.isfile(script):
        ...
        if mode in ("true", "1", "yes"):
            mode = "on"
        if mode not in ("on", "off"):
            return {"ok": False, "message": f"Unknown screensaver mode {mode}"}
        try:
            proc = _run_as_deck(
                ["python3", script, "set-screensaver", "--mode", mode],
                timeout=25,
            )
        except subprocess.TimeoutExpired:
            return {"ok": False, "message": "set-screensaver timed out"}
        return _json_from(proc)
