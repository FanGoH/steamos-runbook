...
| HDMI / top tap | Same warp + `XSendEvent` onto Cemu TV / Azahar Primary, display 0 only (`abs_targets_hdmi_surface`). Do not take this path for Odin GamePad-only (`primary_from_secondary`). Host uinput does not reach wx/GTK on `:1`. GamePad path stays display 1 |
