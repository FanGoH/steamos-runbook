...
| Host ELF | `~/.local/bin/sunshine-ds-kms` git **`dca4efd5`** (Eden HDMI trackpad on `4ca50111` GamePad), `cap_sys_admin=ep`, RUNPATH `~/.local/lib/sunshine-ds-kms`. Sidecar must not write `node=` |
...
| [FanGoH/Sunshine](https://github.com/FanGoH/Sunshine) | `cursor/eden-hdmi-touch-f15e` | `dca4efd5` | Eden HDMI top taps are a relative trackpad; Cemu/Azahar HDMI + overlay stay abs; GamePad path unchanged |
