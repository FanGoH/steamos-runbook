...
| Game Mode `:48200` **Cemu audible, Steam menu silent** | Steam has `STEAM_DISABLE_AUDIO_DEVICE_SWITCHING=1` and plays UI to the Pulse default HDMI sink. A pinned `audio_sink = Virtual Surround Sound` records Cemu only. `write_kms_conf` pins the HDMI alsa sink. If log is `Found default monitor …hdmi….monitor` then `PulseAudio record stream not ready`, the HDMI sink was SUSPENDED (no continuous player). Live ELF must unsuspend that sink and **not** set `PA_STREAM_DONT_INHIBIT_AUTO_SUSPEND`. |
