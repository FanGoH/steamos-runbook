...
- Mux mutes **and EVIOCGRAB**s P1/P2 while `FOCUSED_APP=769` / overlay / QAM **or no Cemu/Azahar/Eden is running**, so Steam menus and native games (Dusklight) only see the Sunshine pad (P1 copying Thor was duplicate input). Plugin `pads` list never includes sinks (`1209:e301` / `e302`) or Steam wraps (`28de:11ff`) when Thor/Odin are present. Shared / Multiplayer must persist; a status poll must not flip the toggle back to Shared P1.
