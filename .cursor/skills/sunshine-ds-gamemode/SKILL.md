...
- **4K HDMI + 1080p GamePad bottom taps**: GamePad View stays 1920×1080 at `0,0` on nested `:1` (now 3840×2160). Inject warps **into that window** and XSendEvents the GL child. Do **not** also uinput. `GAMESCOPE_FOCUS_DISPLAY` middle must stay **1** while Cemu is on `:1` (live tuple e.g. `12602, 1, 68`). Middle **0** after a kms restart / Steam leaves mouse on `:0` and GamePad taps do nothing — `cover_gamepad_under_tv` and GamePad inject rewrite only the middle. Do not write a stale `12346,1,66`. Thor’s bottom panel is **1240×1080**; live packets are `ref=1239x1079`. `packet_to_unit` unletterboxes 16:9 inside that rectangle (visible top is packet y≈191, not 0). A 16:9 stream-sized ref is a no-op. Do **not** run `client_to_touchport`. Log: `GamePad pkt … unit=…` then `abs … 1920x1080`.
