#!/usr/bin/env bash
# Install / refresh the Second Screen Decky plugin (dual-screen toggle + windows).
...
if ! python3 "$ROOT/scripts/second-screen-windows.py" self-test >/dev/null; then
  echo "second-screen-windows self-test failed."
  exit 1
fi

SRC="$ROOT/decky/SecondScreen"
...
if [ ! -d "$PARENT" ]; then
  ...
  record_manual "Install Decky Loader, then Second Screen" <<EOF
# After Decky exists:
$ROOT/scripts/ensure-second-screen-decky.sh
EOF
  exit 2
fi
...
if [ -w "$PARENT" ]; then
  copy_plugin
  finish_install "Installed Second Screen to $PLUGIN_DEST"
fi
...
if [ -w "$PLUGIN_DEST/main.py" ] && [ -w "$PLUGIN_DEST/dist/index.js" ]; then
  ...
  finish_install "Updated writable Second Screen files in $PLUGIN_DEST"
fi
...
if sudo -n true 2>/dev/null; then
  ...
  finish_install "Installed Second Screen to $PLUGIN_DEST (sudo)"
fi
