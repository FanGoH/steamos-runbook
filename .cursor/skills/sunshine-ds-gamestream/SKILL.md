...
| Game Mode `:48200` **Cemu boot is black, no Steam Launching logo** | The 10×10 InputOnly stub was tagged `STEAM_GAME` / `FOCUSED_APP=<shortcut>`. gamescope presents that empty window. Keep `FOCUSED_APP=769` and Steam BPM as baselayer until Cemu is ≥64×64. Bottom idle clock stays until `ffplay` is mapped. |
