...
info="$(curl -sS --max-time 3 "$KMS_URL/serverinfo" 2>/dev/null || true)"
if [ -z "$info" ]; then
...
print(f"state={g('state')!r}")
print(f"currentgame={g('currentgame')!r}")
print(f"uniqueid={g('uniqueid')!r}")
PY
)"
...
if [ "$state" = "SUNSHINE_SERVER_BUSY" ] && [ "$currentgame" != "0" ]; then
  ...
  sleep 0.4
  info="$(curl -sS --max-time 3 "$KMS_URL/serverinfo")"
  eval "$(python3 - "$info" <<'PY'
import sys, xml.etree.ElementTree as ET
r = ET.fromstring(sys.stdin.read())
def g(t):
...
leftover="$(ps -T -o comm= -p "$kms" | grep -E 'session::audio|session::video|stream::control' || true)"
if [ -n "$leftover" ]; then
