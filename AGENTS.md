...
- Sunshine / sunshine-ds GameStream debug: read `.cursor/skills/sunshine-ds-gamestream/SKILL.md` before tracing 503, black capture, or “Starting Desktop” from scratch. Do not mix Decky `:47989` with dev sunshine-ds HTTP. Restart DS by `pgrep -x sunshine-ds` pid only. Desktop Cemu dual-screen (TV on HDMI, GamePad on Virtual-sunshine-ds): `.cursor/skills/cemu-dual-screen/SKILL.md` and `scripts/ensure-cemu-dual-screen.sh`.
