...
- Switch on this box: install Eden into RetroDECK’s **user** slot (`/var/data/retrodeck/external_components/eden/`) via `scripts/ensure-eden-component.sh` from `~/AppImages/eden.appimage`, and a shim at `external_components/ryubing/` because bundled linux `es_systems.xml` still lists `%EMULATOR_RYUBING%` first and `run_game.sh` (Steam/Tender shortcuts without `-e`) uses that file, not `custom_systems`. Do **not** use [iAbuser/RetroInjector](https://github.com/iAbuser/RetroInjector). ES-DE UI must use `%EMULATOR_EDEN%` — **not** `%EMULATOR_FLATPAK-SPAWN% --host`. On each Eden launch, bind player 0 to the current joystick (prefer physical Xbox `045e:*` that is not Sunshine, then Switch Pro, then Steam virtual `28de:11ff`, then Sunshine; skip ASRock LED). Do **not** inherit Steam’s `SDL_GAMECONTROLLER_IGNORE_DEVICES`. Use `ALLOW_STEAM_VIRTUAL_GAMEPAD=1`, hidapi off, `IGNORE_DEVICES_EXCEPT` for Steam virtual + Xbox + Switch Pro, `enable_joycon_driver=false`. If no pad is present yet, keep the last GUID. Keep **borderless** (`fullscreen_mode=0`, `fullscreen=true`) plus async shaders. RetroDECK does **not** copy the cart into RAM (Flatpak bind-mounts the same inode). RSS is Eden guest DRAM: global `memory_layout_mode=2` is **8GB**, and Engage’s per-game 4GB was ignored (`use_global=true`). Pin 4GB on `custom/0100A6301214E000.ini` before boot. Swap is already 7.3G zram + 1G file; zram is compressed RAM so it does not give a free 15G dump buffer. Eden `-g` is the same `BootGame` as clicking the list, but it runs in the MainWindow constructor **before** `show()` — that is the Steam Launching hang, not a RetroDECK memcpy. Tender `rom-launcher` wrap skips RetroDECK for Switch dumps **over 8GiB** and execs the host AppImage `-f -g` (borderless). `ensure-eden-component.sh` also symlinks `~/retrodeck/roms/switch/*/` into `~/emulation/switch/games/` (no copy). Re-install the wrap from `ensure-eden-component.sh` after a Tender plugin update. A Switch `.m3u` is unused (Tender/ES-DE ignore it, Eden applies sibling update NSPs). If `%ROM%` is a folder, pick the largest `.xci`, not a DLC `.nsp`.
