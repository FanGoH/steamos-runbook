...
Host/capture/screensaver recipe is `.cursor/skills/sunshine-ds-gamemode/SKILL.md` (`checkpoint-2026-09-10-gamemode-tender-ds`). Do **not** run the KWin desktop script in Game Mode. Use `scripts/ensure-azahar-gamemode-dual-screen.sh` (standalone Azahar, SteamLaunch, Secondary Window `ffplay` `x11grab` onto `:2`). Tender 3DS Play while `:48200` is BUSY + virtual sidecar skips RetroDECK `azahar-launcher` and execs that script with the dump (`AZAHAR_PAD_MATCH` default **Thor** on that path). Local Play (kms FREE) stays RetroDECK. Manual default pad is **Odin**. `--attach` waits for an already-open Azahar (no second SteamLaunch). Restart Azahar after a GUID change. After `ensure-sunshine-ds-gamemode.sh --start`, re-run `--mirror-only` (kms recreates `:2` and the idle screensaver covers the 3DS bottom). On Azahar/mirror exit, leftover `:2` x11grab must die then `--paint` or the bottom stays frozen. Overlay is the Cemu recipe when Steam Big Picture is mapped; otherwise sunshine-ds sends `steam://overlay/toggle`. The Game Mode focus watcher must **not** reclaim Primary on `FOCUSED_APP=769` without `STEAM_OVERLAY` (that steals overlay ~2s later). Do **not** start `steam-guide-from-select.py`.

Process `comm` is `azahar`. `resourceClass` is `Azahar`. Dual-screen windows have no close button; 3DS Home does not quit Azahar. Playbook `reaper SteamLaunch` is a **systemd --user** child, so Steam Exit never SIGTERMs it — that is the OoT “Exiting…” hog. Overlay/QAM only SIGSTOP; Exit (`FOCUSED_APP=769`, no overlay, no QAM blur) runs `ensure-azahar-gamemode-dual-screen.sh --quit` after ~2s. Moonlight **Quit game** or `scripts/sunshine-app-stop.sh azahar` / `--quit`. Do not `pkill -f` sunshine. Do not wait for a Sunshine pad before launching.
