...
| Overlay | Hold Select 0.5s → `STEAM_OVERLAY=1` on BPM + `FOCUSED_APP=769` | Same; hide restores Cemu TV. Overlay lists Cemu TV + GamePad View. Focus watcher must not reclaim while overlay is up, and must **not** reclaim on `FOCUSED_APP=769` without the overlay atom (that steals HDMI back ~2s later). Cemu / Azahar / Eden are **SIGSTOP**’d so they do not eat pad input (`scripts/inhibit-emu-input-on-steam-ui.py`). SIGCONT if Exit `SIGTERM` is pending. After the emulator dies, restore `FOCUSED_APP=769` on `:0`/`:1` or Steam stays on Exiting (leftover Wind Waker on `:1`). Steam keeps the Sunshine pad for overlay / qAM. |
