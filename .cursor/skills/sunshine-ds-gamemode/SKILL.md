...
**Checkpoint `checkpoint-2026-09-14-gamemode-fill-exit`:** Tender Cemu/Azahar tiles while `:48200` is BUSY fill Thor bottom (`ffplay` `scale=1920:1080` onto `:2`; Moonlight GamePad Stretch/Fit on a 1080p stream, `faf41b72`, sideloaded Thor/Odin/phone). HDMI stays native (4K). Azahar Steam Exit hops `--paint` after dropping a leftover `sunshine-ds-bottom-paint` fragment so the clock is not in `app-steam-app*.scope`. Tile grab remains `checkpoint-2026-09-11-gamemode-tender-ds` (`--attach`, no `-f`, x11grab session `:1`). 3DS dumps `checkpoint-2026-09-11-3ds-n3ds`. Home/Library mute `checkpoint-2026-09-11-steam-menu-mute`. Mux `checkpoint-2026-09-11-emupads-mux`. Local Play (kms FREE) stays `-f`. Steam **audio** menu clicks still **pending**.
...
- **4K HDMI + 1080p `:2`:** Resolutions **coexist**. HDMI size is Steam scanout **`:0` only** (`gamescope_hdmi_tv_size`). Do **not** max() with `:1` — a leftover 4K Cemu window keeps `:1` at 3840 after the TV is 1080p, then attach rewrites the TV to 4K and Moonlight taps miss. Bottom / GamePad / `:2` stay 1920×1080. While HDMI is 1080p, Cemu TV must be 1920×1080 (no FULLSCREEN onto a larger leftover `:1`). 4K HDMI later: `:0` grows and this follows. Do **not** shrink a live 4K `:0` to 1080p just to match `:2`, and do **not** grow `:2` to 4K. Do **not** use EDID preferred 4K while the session is still 1080p (Cemu/Azahar `windowsize` 3840×2160 flickers and leaves a brighter 1080p tile). Moonlight **GamePad fill** Stretch vs Fit decides how 1080p lands on Thor’s bottom. **Refresh:** both panels are 60Hz. Cap **both** encode clocks to `min(HDMI fps, :2 fps, 60)` — not only the GamePad stream. A 120fps software x264 sibling (`8.333ms`) flickers 4K scanout. Set Odin/Thor Moonlight to 60 FPS too. **Do not** run `fix-4k-scanout` from QAM status (clears `COMPOSITE_FORCE` and clips Steam `3840×2161` while overlay/QAM need compose — that breaks Guide / Hold-Select). Clip only on virtual start/stop when overlay is down. Hold-Select hide must find Cemu TV on `:1` first (`tv=0` on `:0` leaves overlay unrestored). Pause is **local TV only** — Odin GamePad needs the sidecar (`serial=` + `pw_node=`). `--start` still brings `:2` up while `:48200` is `BUSY` even if Pause is set. Local 4K TV: QAM **Virtual second display → Pause**. `:1` can stay 1080p (a native game) while `:0` is 4K. Emulator Dual-screen Auto is a separate `mux.json` layout toggle. Steam performance overlay **66fps on a 60Hz TV** is mangoapp compositor/UI fps, not the panel. HDMI feedback is 60. Headless `:2` must pass `-r 60`. Do not restart `gamescope-session` to “fix” the overlay.
