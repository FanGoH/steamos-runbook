...
- Gate A (Grip/Order pair): host override enabled → Switch 2 Change Grip/Order → `scripts/nuxbt-run.sh demo` (sets `DBUS_SYSTEM_BUS_ADDRESS` to host `/run/host/run/dbus/...`; plain `distrobox … nuxbt demo` fails without that).
