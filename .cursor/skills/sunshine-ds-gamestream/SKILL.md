...
| Game Mode `:48200` HDMI **went black after reconnect**, GamePad still fine, log `GL: graphics.cpp:664: [00000501]` | Second Moonlight session rebuilt kmsgrab without `eglMakeCurrent` on the capture thread (pwgrab already binds its context). DCC import then `GL_INVALID_VALUE`; a process-static shader from the first session is also dead. Fix: MakeCurrent every HDMI snapshot and keep the download program on `display_ram_t`. Fresh `sunshine-ds-kms` pid restores picture until the next reconnect. Stage the ELF after `119d7452` that includes this bind. |
