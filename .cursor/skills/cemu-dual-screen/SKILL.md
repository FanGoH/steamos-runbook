...
User confirmed 2026-09-11: “amazing, cemu works correctly” — Tender Cemu **tiles** while streaming `:48200` put GamePad on the Thor bottom (`ffplay` x11grab from session `:1`, not `:0`). Overlay/QAM/Home mute EmuPads sinks; Steam Exit `--quit`s on SIGTERM only (`checkpoint-2026-09-11-emupads-mux`). Earlier `checkpoint-2026-09-10-gamemode-tender-ds` attached but grabbed `:0` for a `:1` xid. Do not “improve” this unless it breaks. Full recipe: `.cursor/skills/sunshine-ds-gamemode/SKILL.md`. Screens-only subset: `checkpoint-2026-09-10-gamemode-dual-stream`. Sep 9 `checkpoint-2026-09-09-gamemode-cemu-ds` is the HDMI DCC / Cemu-picture baseline.
