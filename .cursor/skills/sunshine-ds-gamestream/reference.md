...
| HDMI / top tap | Display 0: Cemu TV on `:1` while playing; Steam Big Picture on `:0` while hold-Select overlay is up (`STEAM_OVERLAY=1`, log `HDMI inject: overlay on :0` / `overlay-abs`). Do not take this path for Odin GamePad-only. GamePad path stays display 1. Live 17:34 in-game: `HDMI inject: using :1` TV `0x80003f` 1920×1051; bottom `GamePad inject: using :1` `0x800040` |
