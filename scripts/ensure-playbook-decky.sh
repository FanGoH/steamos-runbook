#!/usr/bin/env bash
# Install / refresh the Playbook Decky plugin (QAM post-update.sh).
...
if ! python3 "$ROOT/scripts/test_playbook_post_update.py" >/dev/null; then
...
PLUGIN_DEST="${DECKY_HOMEBREW_DIR:-/home/$STEAMOS_USER/homebrew}/plugins/Playbook"
SNAPSHOT="${DECKY_HOMEBREW_DIR:-/home/$STEAMOS_USER/homebrew}/data/Playbook"
...
if [ ! -d "$PARENT" ]; then
  ...
  record_manual "Install Decky Loader, then Playbook" <<EOF
# After Decky exists:
$ROOT/scripts/ensure-playbook-decky.sh
EOF
  exit 2
fi
...
if [ -w "$PARENT" ]; then
  copy_plugin
  finish_install "Installed Playbook to $PLUGIN_DEST"
fi
...
if [ -w "$PLUGIN_DEST/main.py" ] && [ -w "$PLUGIN_DEST/dist/index.js" ]; then
  ...
  finish_install "Updated writable Playbook files in $PLUGIN_DEST"
fi
...
if sudo -n true 2>/dev/null; then
  ...
  finish_install "Installed Playbook to $PLUGIN_DEST (sudo)"
fi
