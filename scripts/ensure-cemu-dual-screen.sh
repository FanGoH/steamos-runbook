...
# Idempotent. Does not launch a ROM unless CEMU_ROM is set and Cemu is not running.
set -uo pipefail
...
if ! ps -eo comm= | grep -qx cemu && ! ps -eo comm= | grep -qx Cemu; then
  if [ -n "${CEMU_ROM:-}" ]; then
    echo "Cemu is not running. Launch with CEMU_ROM when ready (script does not auto-start a game)."
  fi
