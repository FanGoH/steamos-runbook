...
- Sunshine: disable Flatpak user-unit autostart; Decky Sunshine is the only starter (same Flatpak install). Clear stale `SUNSHINE_SERVER_BUSY` (Moonlight 503) with `POST /api/apps/close` — do **not** `/api/restart` the Decky setuid instance (it dies and leaves a defunct bwrap).
