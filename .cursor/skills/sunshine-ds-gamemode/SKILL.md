...
- **4K HDMI + 1080p GamePad 1/4 flicker** is gamescope BASELAYER slipping to GamePad View (`1920×1080+0+0` is the top-left quarter of `3840×2160`) or the Cemu watcher restacking TV/GamePad every tick because a withdrawn Tk clock still matched `xdotool search`. Recover drops that clock while the mirror is live; the watcher only xprops TV BASELAYER when GamePad stole it — no `windowraise` / `windowsize` loop.
