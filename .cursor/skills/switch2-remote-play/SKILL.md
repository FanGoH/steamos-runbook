...
**Input bridge:** `scripts/nuxbt-bridge.sh` → `scripts/nuxbt-sunshine-bridge.py`. Reads Sunshine libvirtualhid pad (no `EVIOCGRAB`), maps face buttons by position, **HOME = LB+RB+Start** (Guide still works), feeds NUXBT at 120 Hz. EmuPads off for the session. NUXBT is **Bluetooth HID only** — the USB stick is the host BT radio, not a USB link to the Switch.
