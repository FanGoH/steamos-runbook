...
| Overlay | Hold Select 0.5s → `STEAM_OVERLAY=1` on BPM + `FOCUSED_APP=769` | Same; hide restores Cemu TV. **QAM** is Quick Access (`...`): `GAMESCOPE_BLUR_MODE!=0`, FOCUSED_APP stays the game — do not treat 769 as QAM, do not `keepAbove`/raise the emu over that panel. Overlay lists Cemu TV + GamePad View. Focus watcher must not reclaim while overlay or QAM is up. Cemu / Azahar / Eden are **SIGSTOP**’d so they do not eat pad input (`scripts/inhibit-emu-input-on-steam-ui.py`). SIGCONT if Exit `SIGTERM` is pending. After the emulator dies, restore `FOCUSED_APP=769` on `:0`/`:1` or Steam stays on Exiting (leftover Wind Waker on `:1`). Steam keeps the Sunshine pad for overlay / QAM. |
