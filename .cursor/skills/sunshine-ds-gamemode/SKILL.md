...
- **4K HDMI + 1080p GamePad 1/4 flicker** (Thor clip `screen-20260917-140731`, **also in the Steam menu**): not Cemu-specific. `mangoapp overlay window` on `:0` is `GAMESCOPE_EXTERNAL_OVERLAY` at **1920×1080** while HDMI/Steam BPM is 4K — steamcompmgr paints that as a quarter overlay even with no game. `xdotool windowsize` does not stick (mangoapp resets). Zero-opacity overlays are not painted; `gamescope_hide_undersized_hdmi_overlays` (recover watch) sets opacity 0 when mango is smaller than HDMI. Do not `systemctl stop` mangoapp (`Restart=always`). Nested `:1` must stay HDMI native — shrinking it to 1080p quarters Steam itself. Cemu GamePad is a second 1080p focus candidate on `:1`; overlay-tag + opacity 0 + `windowraise` (not `windowactivate`). Do not park at `3840,0`. Stamp BASELAYER on **`:0`**. Do not set MODE_CONTROL idx 2.
