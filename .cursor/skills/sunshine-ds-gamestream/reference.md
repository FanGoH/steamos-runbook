...
| [FanGoH/Sunshine](https://github.com/FanGoH/Sunshine) | `cursor/bottom-touch-4k-f15e` **`4ca50111`** (XTest GamePad taps + linear Stretch `packet_to_unit`). HDMI Eden top taps (`Eden | v…`, Qt XTEST) are **`abce4993`** on `cursor/eden-hdmi-touch-f15e`. AUTOCONNECT + object.serial remains `9e07d39e` — do not ship `f8d9968c` skip-AUTOCONNECT |
...
| Host ELF | `~/.local/bin/sunshine-ds-kms` git **`abce4993`** (Eden HDMI XTEST on `4ca50111` GamePad), `cap_sys_admin=ep`, RUNPATH `~/.local/lib/sunshine-ds-kms`. Sidecar must not write `node=` |
