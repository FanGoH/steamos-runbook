...
Host/capture/screensaver recipe is `.cursor/skills/sunshine-ds-gamemode/SKILL.md` (`checkpoint-2026-09-11-gamemode-tender-ds`). Do **not** run the KWin desktop script in Game Mode. Use `scripts/ensure-azahar-gamemode-dual-screen.sh` (standalone Azahar, SteamLaunch, Secondary Window `ffplay` `x11grab` onto `:2`). Tender 3DS Play while `:48200` is BUSY + virtual sidecar skips RetroDECK `azahar-launcher` and **runs** that script with the dump (`AZAHAR_PAD_MATCH` default **Thor** on that path). Do **not** `exec` it — `rom-launcher` waits for `azahar` then `--quit`/`--paint`. Local Play (kms FREE) stays RetroDECK. Manual default pad is **Odin**. `--attach` waits for an already-open Azahar (no second SteamLaunch). Restart Azahar after a GUID change. After `ensure-sunshine-ds-gamemode.sh --start`, re-run `--mirror-only` (kms recreates `:2` and the idle screensaver covers the 3DS bottom). On Azahar/mirror exit, leftover `:2` x11grab must die then `--paint` or the bottom stays frozen. The focus watcher is **disowned**; Tender `rom-launcher` waits for `azahar` then `--quit`s with a timeout (do not `exec` the dual-screen script). `--paint` / xdotool on `:2` must not block Steam Exit. Steam Exit looks for Azahar on `:0` and `:1`. Inhibit paints `:2` when the last emu is gone. Overlay is the Cemu recipe when Steam Big Picture is mapped; otherwise sunshine-ds sends `steam://overlay/toggle`. The Game Mode focus watcher must **not** reclaim Primary on `FOCUSED_APP=769` without `STEAM_OVERLAY` (that steals overlay ~2s later). Do **not** start `steam-guide-from-select.py`.
