...
PLUGIN_OUT="$(distrobox enter "$CONTAINER" -- bash -lc "export PATH='$VENV/bin:\$PATH'; nuxbt check" 2>/dev/null || true)"
echo "Plugin status: $PLUGIN_OUT"

if ! echo "$PLUGIN_OUT" | grep -qi 'Enabled'; then
  record_manual "Enable NUXBT BlueZ override (tmpfs; clears on reboot)" <<EOF
# Puts bluetoothd in --compat --noplugin=* under /run/systemd/.../nuxbt.conf
# Temporarily affects BT HID (keyboards/mice). Optional: stop nso-gc for the test.
systemctl --user stop nso-gc.service 2>/dev/null || true
distrobox enter $CONTAINER -- bash -lc 'export PATH=$VENV/bin:\$PATH; nuxbt toggle'
# Confirm: answer yes to Enable
distrobox enter $CONTAINER -- bash -lc 'export PATH=$VENV/bin:\$PATH; nuxbt check'
EOF
  echo "BlueZ override not enabled yet (needs sudo via nuxbt toggle)."
  exit 2
fi
...
# Or: nuxbt test
# When finished, disable override and restore nso-gc if you use it:
distrobox enter $CONTAINER -- bash -lc 'export PATH=$VENV/bin:\$PATH; nuxbt toggle'
systemctl --user start nso-gc.service 2>/dev/null || true
EOF

echo "Plugin enabled. Open Switch 2 Change Grip/Order, then run nuxbt demo (see manual steps)."
exit 0
