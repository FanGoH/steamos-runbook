...
- Switch on this box: install Eden into RetroDECK’s **user** slot (`/var/data/retrodeck/external_components/eden/`) via `scripts/ensure-eden-component.sh`. Do **not** use [iAbuser/RetroInjector](https://github.com/iAbuser/RetroInjector) (sudo-copies cooker tarballs into the Flatpak `files/` dir; wiped on Flatpak update; does not fix input). ES-DE must use `%EMULATOR_EDEN%` — **not** `%EMULATOR_FLATPAK-SPAWN% --host`. Host spawn drops Game Mode env and makes overlay/Exit attach to RetroDECK while Eden is a foreign process. `%INJECT%` (old Ponzu/Yuzu) only adds per-game argv from a `.esprefix` file. Ponzu is gone as of RetroDECK 0.10. Moonlight/Sunshine regularly destroys Steam’s virtual pad (`hid_read failure`) and injects an Xbox-like device. Do **not** inherit Steam’s `SDL_GAMECONTROLLER_IGNORE_DEVICES` (it hides those Xbox IDs) and do **not** pin player 0 to Cemu’s Steam Virtual GUID. Use `ALLOW_STEAM_VIRTUAL_GAMEPAD=1`, hidapi off, `IGNORE_DEVICES_EXCEPT` for Steam virtual + Xbox, player 0 = SDL port 0 with no GUID, `enable_joycon_driver=false`.
