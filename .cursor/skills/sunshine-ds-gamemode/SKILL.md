...
- **4K HDMI + 1080p `:2`:** `gamescope_hdmi_tv_size` follows live `:0`/`:1`. Do **not** use EDID preferred 4K while the session is still 1080p (Cemu/Azahar `windowsize` 3840×2160 flickers and leaves a brighter 1080p tile in the top-left of a 4K panel). When Steam output is actually 4K, `:0`/`:1` grow and this follows. Bottom / GamePad / `:2` stay 1920×1080 (ffplay scales the grab). Moonlight **GamePad fill** Stretch vs Fit decides how that 1080p lands on Thor’s bottom. Do not request Thor native 1080×1240 or the TV 4K size for video/1. Do not resize `:2` to 4K. Do not change HDMI to match `:2`. **4K HDMI + Odin GamePad:** HDMI / `:0` stay 4K; `:2` stays 1920×1080@60 (`-r 60`). That is the supported dual-stream layout. Odin GamePad-only is PipeWire `:2` as video/0 (not HDMI kmsgrab). A 120fps software x264 pace (`8.333ms`) on that stream flickers the 4K TV — host caps gamescope-virtual at 60. Set Odin Moonlight to 60 FPS too. QAM status re-clips Steam `3840×2161` while BUSY. Pause is **local TV only** — Odin bottom / GamePad needs the sidecar (`serial=` + `pw_node=`). `--start` still brings `:2` up while `:48200` is `BUSY` even if Pause is set, then `fix-4k-scanout` (no 2161 overflow, no `COMPOSITE_FORCE`). Do not resize `:2` to 4K. Local 4K TV: Second Screen QAM **Virtual second display → Pause** writes `~/.config/sunshine-ds-gamemode/virtual-output` (`off`) and `--stop`s headless gamescope. `--start` no-ops while pref is off **and** `:48200` is FREE. Pause also clears `GAMESCOPE_COMPOSITE_FORCE` and clips Steam CEF `3840×2161` to `3840×2160` (1px overflow forces compose and flickers). `:1` can stay 1080p (a native game) while `:0` is 4K — do not windowsize that game to 4K. Emulator Dual-screen Auto is a separate `mux.json` layout toggle. Steam performance overlay **66fps on a 60Hz TV** is mangoapp compositor/UI fps, not the panel. HDMI feedback is 60 (`GAMESCOPE_DISPLAY_REFRESH_RATE_FEEDBACK`, `modes.cfg` `@60`). Headless `:2` must pass `-r 60` (`-o 60` unfocused) so it is not an uncapped GPU clock. This Sony list has no 66Hz mode (Deck OLED does). Do not restart `gamescope-session` to “fix” the overlay.
