...
Read this **before** changing capture, PipeWire, or Cemu launch. Desktop Thor/Odin daily dual-screen stays Plasma `:48100`. Do not merge kms into play / `:48100`. Do **not** merge leftover `cursor/*-f15e` Game Mode history onto `main` — current `main` already has 4K HDMI, XTest GamePad, recover/:2, and Pause. Stack/SHAs: [../sunshine-ds-gamestream/reference.md](../sunshine-ds-gamestream/reference.md). Host uniqueids: `~/.cursor/skills/sunshine-ds-gamestream/host.md`.
...
- **Two Moonlight clients:** closing Thor/Odin must **not** drop the other. Host bug was `session::stop()` using `running_sessions <= 1` (lagged) and `Last Moonlight client gone; closing Desktop`. Count other STARTING/RUNNING peers; close Desktop only when that is 0. Log `CLIENT CONNECTED [AYN_Thor]` / `CLIENT DISCONNECTED [name]`.
- **HDMI-only Azahar touch:** RetroDECK is one stacked window (`Azahar … | GAME`, no Secondary). Display-0 inject must treat that as an HDMI surface (and XSendEvent it). Dual-screen Separate Windows still uses Secondary for bottom / Primary for top.
- **Tender “RomM disconnected”** on a 3DS tile usually means no `rom_installs` row (UI shows Download and hits RomM) even when the `.3ds` is already in `n3ds`. `set-steam-launch-options.py --repair-tender`, then **reopen Tender**. Empty Steam LaunchOptions recover via `SteamAppId` → Tender DB. Do not rewrite `shortcuts.vdf` in Game Mode. Azahar boots **only** `.3ds` / `.cci` / `.cxi` — never `.cia` (NAND installer popup). A missing Luigi CIA must **not** fall through to the largest dump in `n3ds` (that was Ultra Sun). `--repair-tender` **deletes** stale CIA `rom_installs` rows. `--clear-tender-3ds-cache` drops Tender’s 3DS `platform_sync_state` stamp (next sync re-fetches 3DS only; not Force Full Sync). CIA catalog rows stay Download even when a sibling `.3ds` exists.
...
- Mux mutes **and EVIOCGRAB**s P1/P2 while `FOCUSED_APP=769` / overlay / QAM **or no Cemu/Azahar/Eden is running**, so Steam menus and native games (Dusklight) only see the Sunshine pad (glyph flicker / duplicate input was P1 copying Thor). Plugin `pads` list never includes sinks (`1209:e301` / `e302`) or Steam wraps (`28de:11ff`) when Thor/Odin are present. Shared / Multiplayer must persist; a status poll must not flip the toggle back to Shared P1. When a Sunshine pad is present, drop the desk USB Xbox so Eden (NMH3) does not last-activity-steal.
- **QAM Render second screen / Virtual second display** Pause `--stop`s headless `:2` (HDMI stays). If the toggle already looks On but Thor bottom is dead, turn it Off then On, or `python3 scripts/second-screen-windows.py set-virtual-output --mode on`. **Screensaver** Off kills the idle clock without stopping `:2`. `steamos-sunshine-ds-gamemode-recover.service` (`virtual.sh --watch`) restarts `:2` / paints the clock when those prefs are on and the bottom is empty. Live Cemu/Azahar `ffplay` is left alone except **640×480** SDL remaps after kms restart — `present_ffplay_baselayer` `windowsize`s 1920×1080. Leftover `ffplay` with no emu is killed then painted. Do not `--start` kms from that watcher. After `:2` respawns it gets a new PipeWire `serial=`; live kms still AUTOCONNECTs the old node and video/1 falls through to HDMI (Thor bottom duplicates the TV). `start_virtual` then `systemctl --user restart steamos-sunshine-ds-gamemode.service` (`--start-kms` does the same if kms is already up). Keep `:2`. `--start` still brings `:2` up while `:48200` is `BUSY` even if Pause is set. Pause is **local TV only** — Odin GamePad needs the sidecar (`serial=` + `pw_node=`).
- **4K HDMI + 1080p GamePad / `:2`:** Resolutions **coexist**. HDMI size is Steam scanout **`:0` only** (`gamescope_hdmi_tv_size`). Do **not** max() with `:1` — a leftover 4K Cemu window keeps `:1` at 3840 after the TV is 1080p, then attach rewrites the TV to 4K and Moonlight taps miss. Bottom / GamePad / `:2` stay 1920×1080. While HDMI is 1080p, Cemu TV must be 1920×1080 (no FULLSCREEN onto a larger leftover `:1`). 4K HDMI later: `:0` grows and this follows. Do **not** shrink a live 4K `:0` to 1080p just to match `:2`, and do **not** grow `:2` to 4K. Do **not** use EDID preferred 4K while the session is still 1080p. Nested `:1` must stay HDMI native — shrinking it to 1080p quarters Steam itself. GamePad View stays 1920×1080 at `0,0` on nested `:1`. wx/GTK drop `XSendEvent`. Inject briefly clears overlay tags, `XTestFake*` a real click onto the GL child, then re-covers. Do **not** also uinput. `GAMESCOPE_FOCUS_DISPLAY` writes go to session `:0` and must `XFlush`. Middle must stay **1** while Cemu is on `:1`. Thor Stretch fills the **1240×1080** panel; live packets are `ref=1239x1079`. `packet_to_unit` is linear `x/width`. Do **not** unletterbox. **Refresh:** both panels are 60Hz. Cap **both** encode clocks to `min(HDMI fps, :2 fps, 60)`. Set Odin/Thor Moonlight to 60 FPS. **Do not** run `fix-4k-scanout` from QAM status (clears `COMPOSITE_FORCE` and clips Steam `3840×2161` while overlay/QAM need compose). Clip only on virtual start/stop when overlay is down. Hold-Select hide must find Cemu TV on `:1` first. Headless `:2` must pass `-r 60`. `sunshine-ds-gamemode-virtual.sh` starts headless from `$XDG_RUNTIME_DIR/sunshine-ds-gamemode-headless` with its own `mangoapp` ftok file. `scripts/ensure-mangohud-presets.sh` installs `[preset 2]` `inherit` + `horizontal` + `horizontal_stretch=1` + `background_alpha=0.5`. Cemu GamePad overlay-tag frame **and** GL children, opacity 0 on the **frame only**. `windowraise` not `windowactivate`. Stamp BASELAYER on **`:0`**. A kms restart can remap `:2` `ffplay` at **640×480** — `present_ffplay_baselayer` must `windowsize` 1920×1080 even when BASELAYER already points at ffplay.

## What must be true

| Surface | Idle (Desktop `958645192`) | Cemu playing |
|---|---|---|
| HDMI / Thor top (video/0) | Steam Big Picture / focused game | Cemu TV at **live session** size (`gamescope_hdmi_tv_size`; `:1` follows HDMI native). GamePad / `:2` stay 1920×1080; client stretches the bottom. |
...
Cemu is Steam `RunGame` (same as Tender Play). Dual-screen when **either** consume-on-read `logs/cemu-gamemode-ds.want` is fresh **or** Tender Play sees `:48200` `SUNSHINE_SERVER_BUSY` plus the gamescope-virtual sidecar **and** Moonlight is watching the bottom (`scripts/gamemode-second-screen-streaming.sh`, Decky **Second Screen** Auto). QAM lists each connected client as a Field in **Second Screen** (`Thor` · Dual-screen vs `Odin` · top-only). Plugin title is `Second Screen · Thor`. A toast on connect/disconnect (QAM open) says Dual-screen vs HDMI only. Top-only Moonlight stays `-f`. Force Dual-screen / HDMI only from Second Screen QAM (`mux.json` `dual_screen`). Local Play (kms FREE) stays `-f`. `rom-launcher` / `Cemu-wrapper` start `cemu-gamescope-focus.sh` **before** gtk_init (`FOCUSED_APP=<shortcut>`, `FOCUS_DISPLAY` middle **1**). A leftover want file windowed-10×10s every Play — delete it. Tender plugin updates overwrite `rom-launcher`; re-run `ensure-eden-component.sh`. Do **not** rewrite `shortcuts.vdf` in Game Mode for Auto — tiles already `exec` `rom-launcher`, which reads the live kms signal. `steam://`, host `flatpak run`, and RetroDECK `-f` stay a 10×10 InputOnly stub. Wait for the Sunshine pad with `bind-gamepad.py wait-appear --match Thor` if needed, then bind **EmuPads P1** (`apply --emu cemu --force --cemu-p1 gamepad`). Do not write Thor’s GUID into XML. Cemu uuid is `{sdl-index}_{guid}` — a stale `1_<guid>` when P1 is now index 0 means “controllers not mapped.” Cemu reads uuid at start; restart after bind.
