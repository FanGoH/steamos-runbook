...
| Game Mode `:48200` **user saw moving yellow square on blue** (now idle clock) | Dual-stream smoke proven 2026-09-09: KMS HDMI + pwgrab `gamescope-virtual`. Idle video/1 is `scripts/sunshine-ds-bottom-screensaver.py` (clock + “Bottom stream idle”). Cemu/Azahar ffplay replace it; `--paint` restores it without restarting headless gamescope. Helper must keep damaging `:2`. |
