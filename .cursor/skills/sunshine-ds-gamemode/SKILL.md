...
- **4K HDMI + 1080p GamePad 1/4 flicker** (Thor clip `screen-20260917-140731`): HDMI flips between a full 4K Cemu TV and the same 3D view in the top-left quarter. The Cemu watcher treated `:2` `getwindowfocus != ffplay` (steamcompmgr) as “needs layout” and ran `present_dual_layout` every 0.4s — `FULLSCREEN` / `windowactivate` on windowed 4K TV vs 1080p GamePad. Do not relayout HDMI while Cemu is focused; only restart `:2` ffplay if it is actually gone. Opacity 0 on the **wx frame is not enough** — gamescope still draws the 1920×1080 GL child at `0,0`. `x11_hide_xid_from_hdmi` must set opacity on frame **and** children. Do not `windowlower` every 0.4s (restack is another flash). Do not hammer `FOCUS_DISPLAY` every tick — Steam rewrites middle 0 in <50ms and the 0↔1 fight can flash HDMI. `x11_resize_if_needed` only. Launch hammer in `cemu-gamescope-focus.sh` keeps live first/third cardinals (`12602, 1, 68` not hardcoded `12346, 1, 66`).
...
- Opacity 0 on the Cemu/Azahar **frame only** — gamescope still draws the GL child; hide children too
