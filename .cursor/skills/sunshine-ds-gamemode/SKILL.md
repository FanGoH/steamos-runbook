...
**Checkpoint `checkpoint-2026-09-18-gamepad-xtest`:** Thor bottom taps reach Cemu GamePad (XTest after briefly clearing overlay/opacity; `:0` FOCUS_DISPLAY flush). 4K HDMI + 1080p GamePad. Thor Stretch sends `ref=1239x1079` — host maps **linear** `x/width` (do not unletterbox Fit bars; that sheared Y). QAM **preset 2** bar spans the TV. After `:2` respawns, kms rebinds video/1. Tender Cemu/Azahar fill+Exit remains `checkpoint-2026-09-14-gamemode-fill-exit`. Tile grab remains `checkpoint-2026-09-11-gamemode-tender-ds`. Home/Library mute `checkpoint-2026-09-11-steam-menu-mute`. Steam **audio** menu clicks still **pending**. HDMI-hud mangoapp isolation remains `checkpoint-2026-09-17-hdmi-hud`.
