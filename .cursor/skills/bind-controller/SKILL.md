...
- QAM **Emu Pads** section (top): **EmuPads P1 / P2** toggle, same widget as Second Screen. Off unplugs P1/P2 (`mux.json` `enabled: false`). The mux daemon stays; uinput nodes go away. On recreates them. Plugin `main.py` must resolve `~/steamos-playbook` (not a leftover `~/worktrees/*` — `apply` is past the first 8k of `bind-gamepad.py`). Reload the plugin after editing or QAM stays on the old bundle.

Install: `scripts/ensure-emupads-mux.sh` then `scripts/ensure-emu-pads-decky.sh`. Source `decky/EmuPads/` (see that README). `~/homebrew/plugins` is often root-owned — sudo is required to copy. **Reload after every plugin edit** or QAM stays on the old bundle (skill `.cursor/skills/decky-plugins/SKILL.md`).
