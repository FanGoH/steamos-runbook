...
| Game Mode `:48200` HDMI **went black after reconnect**, GamePad still fine, log `GL: graphics.cpp:664: [00000501]` | Last Moonlight client used to join `capture_thread_async`, which destroyed kmsgrab EGL. The next session imported DCC without a bound context (`GL_INVALID_VALUE`, tex 0×0, black TV). GamePad survived because `capture_thread_sync2` stays up. Fix: pin the HDMI capture thread, wait for the next session without dropping kmsgrab, `eglMakeCurrent` every snapshot, per-display download program, treat a 0×0 import as reinit (sunshine-ds `f3844600`). Healthy reconnect log: `HDMI capture idle` then `HDMI capture resumed` — not `GL: graphics.cpp:664`. Live ELF must be that build (`sunshine-ds-kms.new` + setcap); a BUSY `:48200` session still runs the old pid until a restart. |
...
That is SteamLaunch RetroDECK Cemu, `CEMU_GAMEMODE_DS=1`, **no** `-f`, mappings only on `Sunshine (libvirtualhid) AYN_Thor`, GamePad `ffplay` `x11grab` onto headless `:2`. Do not run `ensure-cemu-dual-screen.sh` in Game Mode.
