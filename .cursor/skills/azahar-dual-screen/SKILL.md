...
If `AZAHAR_ROM` is set and Azahar is not running, the script launches. Example dump: `~/retrodeck/roms/n3ds/*.3ds`. Do not commit a personal ROM path. Do not inherit Steam’s ignore list. Do not use RetroDECK `-f` / `azahar-launcher`.

## Game Mode (`:48200`)

Host/capture/screensaver recipe is `.cursor/skills/sunshine-ds-gamemode/SKILL.md` (**THE Game Mode `:48200` standard** `checkpoint-2026-09-18-gamepad-xtest`; HUD subset `checkpoint-2026-09-17-hdmi-hud`; Azahar fill+Exit remains `checkpoint-2026-09-14-gamemode-fill-exit`). Do **not** run the KWin desktop script in Game Mode. Use `scripts/ensure-azahar-gamemode-dual-screen.sh` (standalone Azahar, SteamLaunch, Secondary Window `ffplay` `x11grab` `scale=1920:1080` onto `:2`). Tender 3DS Play while `:48200` is BUSY + virtual sidecar skips RetroDECK `azahar-launcher` and **runs** that script with the dump (`AZAHAR_PAD_MATCH` default **Thor** on that path). Do **not** `exec` it — `rom-launcher` waits for `azahar` then `--quit`/`--paint`. Local Play (kms FREE) stays RetroDECK. Manual default pad is **Odin**. `--attach` waits for an already-open Azahar (no second SteamLaunch). Restart Azahar after a GUID change. After `ensure-sunshine-ds-gamemode.sh --start`, re-run `--mirror-only` (kms recreates `:2` and the idle screensaver covers the 3DS bottom). On Azahar/mirror exit, leftover `:2` x11grab must die then `--paint` (drop leftover `sunshine-ds-bottom-paint` fragment; do not start the clock in `app-steam-app*.scope`) or Steam sits on Exiting. The focus watcher is **disowned**; Tender `rom-launcher` waits for `azahar` then `--quit`s with a timeout (do not `exec` the dual-screen script). `--paint` / xdotool on `:2` must not block Steam Exit. Steam Exit looks for Azahar on `:0` and `:1`. Inhibit paints `:2` when the last emu is gone. Overlay is the Cemu recipe when Steam Big Picture is mapped; otherwise sunshine-ds sends `steam://overlay/toggle`. The Game Mode focus watcher must **not** reclaim Primary on `FOCUSED_APP=769` without `STEAM_OVERLAY` (that steals overlay ~2s later). Do **not** start `steam-guide-from-select.py`.

Process `comm` is `azahar`. `resourceClass` is `Azahar`. Dual-screen windows have no close button; 3DS Home does not quit Azahar. Playbook `reaper SteamLaunch` is a **systemd --user** child, so Steam Exit never SIGTERMs it — that is the OoT “Exiting…” hog. Overlay/QAM/Home mute EmuPads sinks (`checkpoint-2026-09-11-steam-menu-mute`). Steam Exit `--quit`s only on SIGTERM pending — do not treat `FOCUSED_APP=769` as Exit. Moonlight **Quit game** or `scripts/sunshine-app-stop.sh azahar` / `--quit`. Do not `pkill -f` sunshine. Do not wait for a Sunshine pad before launching. Every Azahar launch binds standalone + RetroDECK `qt-config.ini` to EmuPads P1 (`bind-gamepad.py apply --emu azahar`).
