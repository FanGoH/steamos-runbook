...
- KWin desktop capture (`ensure-kwin-screencast.sh`): on AMD GFX12 + KWin 6.7 the screenshot/screencast FBO can be all zeros while the TV still shows the desktop. If a silent Spectacle shot is black, `qdbus org.kde.KWin /Compositor org.kde.kwin.Compositing.reinitialize`. Do **not** `kwin_wayland --replace` from these scripts.
