...
| HDMI / top tap | Same warp + `XSendEvent` onto Cemu TV / Azahar Primary, display 0 only (`abs_targets_hdmi_surface`). Do not take this path for Odin GamePad-only (`primary_from_secondary`). Host uinput does not reach wx/GTK on `:1`. GamePad path stays display 1. Live 2026-09-10 17:34: `HDMI inject: using :1` then `HDMI abs … px=1152,511 xid=8388671 1920x1051` (TV GL child `0x80003f`). Bottom still `GamePad inject: using :1` on `0x800040`. ADB `input -d 0 tap 200 200` / `1700 900` warped `:1` to those TV pixels |
