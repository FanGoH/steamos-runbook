#!/usr/bin/env bash
# Install / refresh the Pad Hide Decky plugin (per-pad kernel unplug toggles).
...
"$ROOT/scripts/ensure-hide-controllers.sh" || true

SRC="$ROOT/decky/PadHide"
if [ ! -f "$SRC/main.py" ] || [ ! -f "$SRC/plugin.json" ] || [ ! -f "$SRC/dist/index.js" ]; then
  echo "Missing plugin files under $SRC"
  exit 1
fi

PLUGIN_DEST="${DECKY_HOMEBREW_DIR:-/home/$STEAMOS_USER/homebrew}/plugins/PadHide"
SNAPSHOT="${DECKY_HOMEBREW_DIR:-/home/$STEAMOS_USER/homebrew}/data/PadHide"
...
snapshot_backend() {
  ...
  cp -a "$ROOT/scripts/hide-controllers.py" "$SNAPSHOT/scripts/"
  cp -a "$ROOT/scripts/hide-controllers-sysfs.sh" "$SNAPSHOT/scripts/"
}
...
if [ ! -d "$PARENT" ]; then
  ...
  record_manual "Install Decky Loader, then Pad Hide" <<EOF
# After Decky exists:
$ROOT/scripts/ensure-pad-hide-decky.sh
EOF
  exit 2
fi
...
if [ -w "$PARENT" ]; then
  copy_plugin
  finish_install "Installed Pad Hide to $PLUGIN_DEST"
fi
...
if [ -w "$PLUGIN_DEST/main.py" ] && [ -w "$PLUGIN_DEST/dist/index.js" ]; then
  ...
  finish_install "Updated writable Pad Hide files in $PLUGIN_DEST"
fi
...
if sudo -n true 2>/dev/null; then
  ...
  finish_install "Installed Pad Hide to $PLUGIN_DEST (sudo)"
fi
