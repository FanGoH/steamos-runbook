...
| Game Mode `:48200` HDMI **went black after reconnect**, GamePad still fine, log `GL: graphics.cpp:664: [00000501]` | Last client joined `capture_thread_async` and destroyed kmsgrab EGL. The next session ran `glEGLImageTargetTexture2DOES` with **no current context** (`tex_t::make` + import on a new capture thread). First session works because `init()` left the context current. GamePad survives (`capture_thread_sync2`). Fix: pin HDMI capture, `eglMakeCurrent` every snapshot, fail closed on GL bind errors, skip a bad frame instead of creating a new GBM context. **The 21:36 reconnect was pid 871378 started 21:12 — it never exec'd this ELF.** Check `ps -o lstart -p "$(pgrep -x sunshine-ds-kms)"` vs the binary mtime. Healthy reconnect: `HDMI capture idle` then `HDMI capture resumed`, `[kmsgrab] DMA-BUF copied`, I-frame ~17–23KB. |
