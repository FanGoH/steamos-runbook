...
| Cemu | Tender Play while `:48200` BUSY + virtual sidecar → `CEMU_GAMEMODE_DS=1` + `--attach` (**no** `-f`). Manual: `CEMU_PAD_MATCH=Thor ./scripts/ensure-cemu-gamemode-dual-screen.sh`. TV + GamePad View on session `:1` (`FOCUS_DISPLAY=1`). `ffplay -window_id` **must** `-i :1.0` (not `:0.0`) onto `:2` with `-vf scale=1920:1080`. HDMI TV is native (4K). Do not `$(find_pad_wid)` — that drops `TV_DISPLAY`. Exit: `--quit` on first tick, then `--paint` |
